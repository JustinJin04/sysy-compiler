...
class SymbolTables {
 public:
  ...
  std::vector<std::unordered_map<std::string, std::string>>
      func_sym_table_stack;

  std::vector<std::unordered_map<std::string, ConstArrInfo>>
      const_arr_sym_table_stack;
  std::vector<std::unordered_map<std::string, VarArrInfo>>
      var_arr_sym_table_stack;

  std::vector<std::unordered_map<std::string, PtrInfo>> ptr_sym_table_stack;

  enum class SymbolKind { CONST, VAR, FUNC, CONST_ARR, VAR_ARR, PTR };
  ...
  void insert_to_top(const std::string& ident, const std::string& type_name,
                     int dims, SymbolKind kind) {
  ...
  // for example, array name is arr, let symbol table decide whether arr_1,
  // arr_2, arr_3, ... to store
  void insert_to_top(const std::string& ident, const std::vector<int>& dims,
                     const std::vector<int>& data, SymbolKind kind) {
  ...
  void insert_to_top(const std::string& ident, const std::vector<int>& dims,
                     const std::vector<std::variant<Exp*, int>>& data,
                     SymbolKind kind) {
  ...
  //   assert(kind == SymbolKind::PTR);
  //   PtrInfo info;
  //   // note that we cannot append count behind sym_name, since it has been
  //   declared at funcdef info.sym_name = "@" + ident; info.type_name =
  //   type_name; ptr_sym_table_stack.back()[ident] = info;
  // }
  ...
  SymbolKind find(const std::string& ident) {
    ...
    for (int i = layer_num - 1; i >= 0; --i) {
      if (const_sym_table_stack[i].find(ident) !=
          const_sym_table_stack[i].end()) {
        return SymbolKind::CONST;
      }
      if (var_sym_table_stack[i].find(ident) != var_sym_table_stack[i].end()) {
        return SymbolKind::VAR;
      }
      if (func_sym_table_stack[i].find(ident) !=
          func_sym_table_stack[i].end()) {
        return SymbolKind::FUNC;
      }
      if (const_arr_sym_table_stack[i].find(ident) !=
          const_arr_sym_table_stack[i].end()) {
        return SymbolKind::CONST_ARR;
      }
      if (var_arr_sym_table_stack[i].find(ident) !=
          var_arr_sym_table_stack[i].end()) {
  ...
  bool find(const std::string& ident, SymbolKind type) {
    switch (type) {
      case SymbolKind::CONST: {
        for (auto it = const_sym_table_stack.rbegin();
             it != const_sym_table_stack.rend(); it++) {
      ...
      case SymbolKind::VAR: {
        for (auto it = var_sym_table_stack.rbegin();
             it != var_sym_table_stack.rend(); it++) {
          if (it->find(ident) != it->end()) {
            return true;
          }
        }
      ...
      case SymbolKind::FUNC: {
        for (auto it = func_sym_table_stack.rbegin();
             it != func_sym_table_stack.rend(); it++) {
          if (it->find(ident) != it->end()) {
            return true;
          }
        }
      ...
      case SymbolKind::CONST_ARR: {
        for (auto it = const_arr_sym_table_stack.rbegin();
             it != const_arr_sym_table_stack.rend(); it++) {
          if (it->find(ident) != it->end()) {
            return true;
          }
        }
      ...
      case SymbolKind::VAR_ARR: {
        for (auto it = var_arr_sym_table_stack.rbegin();
             it != var_arr_sym_table_stack.rend(); it++) {
          if (it->find(ident) != it->end()) {
            return true;
          }
        }
      ...
      case SymbolKind::PTR: {
        for (auto it = ptr_sym_table_stack.rbegin();
             it != ptr_sym_table_stack.rend(); it++) {
          if (it->find(ident) != it->end()) {
            return true;
          }
        }
  ...
  bool find_in_current(const std::string& ident, SymbolKind type) {
    switch (type) {
      case SymbolKind::CONST: {
        return const_sym_table_stack.back().find(ident) !=
               const_sym_table_stack.back().end();
      }
      case SymbolKind::VAR: {
        return var_sym_table_stack.back().find(ident) !=
               var_sym_table_stack.back().end();
      }
      case SymbolKind::FUNC: {
        return func_sym_table_stack.back().find(ident) !=
               func_sym_table_stack.back().end();
      }
      case SymbolKind::CONST_ARR: {
        return const_arr_sym_table_stack.back().find(ident) !=
               const_arr_sym_table_stack.back().end();
      }
      case SymbolKind::VAR_ARR: {
        return var_arr_sym_table_stack.back().find(ident) !=
               var_arr_sym_table_stack.back().end();
      }
      case SymbolKind::PTR: {
        return ptr_sym_table_stack.back().find(ident) !=
               ptr_sym_table_stack.back().end();
      }
      default: {
        assert(0);
      }
   ...
   */
  std::variant<Exp*, int> get_array(const std::string& ident,
  ...
  void push_table() {
    ...
    var_sym_table_stack.push_back(
        std::unordered_map<std::string, std::string>());
    func_sym_table_stack.push_back(
        std::unordered_map<std::string, std::string>());
    const_arr_sym_table_stack.push_back(
        std::unordered_map<std::string, ConstArrInfo>());
    var_arr_sym_table_stack.push_back(
        std::unordered_map<std::string, VarArrInfo>());
